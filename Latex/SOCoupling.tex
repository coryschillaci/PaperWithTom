...
\affiliation{
Institute for Advanced Simulation, Institut f\"{u}r Kernphysik and J\"{u}lich Center for Hadron Physics, Forschungszentrum J\"{u}lich, DÐ52425 J\"{u}lich, Germany
}
%\affiliation{Institut f\"{u}r Kernphysik and J\"{u}lich Center for Hadron Physics,
%Forschungszentrum J\"{u}lich, DÐ52425 J\"{u}lich, Germany}%Lines break automatically or can be forced with \\
...
Cold atomic gases with spin-orbit coupling (SOC) have recently been an area of intense interest because of the potential to simulate interesting physical systems with precisely tunable interactions \cite{nature11841}. In condensed matter physics, spin-orbit couplings are essential for many exotic systems such as topological insulators \cite{das2013engineering,PhysRevLett.105.255302}, the quantum spin hall effect\cite{nature12185}, and spintronics \cite{RevModPhys.76.323}. The experimental setup which induces spin-orbit coupling is intimately related to simulation of synthetic gauge fields \cite{RevModPhys.83.1523,hamner2014dicke}. Because they are parity-violating like the weak force, spin-orbit couplings feature prominently in nuclear physics where they are an essential ingredient in realistic effective nuclear forces. Atomic gases provide an excellent testing ground both to explore these real life systems and create new types of spin-orbit coupling which are not yet known to exist in other materials but are interesting in their own right. 

SOC was first realized in a Bose condensate of $^{87}$Rb \cite{nature09887} and extended shortly after to Fermi gases of $^{40}$K \cite{PhysRevLett.109.095301} and $^6$Li \cite{PhysRevLett.109.095302}. These spin-orbit interactions are `synthetic' in the sense that a subset of the hyperfine states stand in as virtual spin states. A particularly interesting consequence of these atomic Hamiltonians is the possibility of bosonic systems with synthetic spin-$1/2$ spin orbit interactions. Although we do not consider this case in our paper, the generalization is straightforward. Henceforth we will simply refer to these degrees of freedom as `spin' and use the standard notation for spin quantum numbers. 
...
In this paper we consider three different types of spin-orbit coupling (SOC). The form of spin-orbit coupling realized in experiments is a linear combination of the Rashba \cite{0022-3719-17-33-015} and linear Dresselhaus \cite{PhysRev.100.580} types,
\begin{align}
...
\end{equation}
For the spin-orbit term $V_{SO}$, we consider equal Rashba and Dresselhaus \eqref{eq:R=D}, pure Rashba \eqref{eq:Rashba}, and Weyl spin-orbit couplings\eqref{eq:Weyl} because these are generally considered to be experimentally feasible.
...
\end{align}
where $U(a,b,x)$ is Kummer's confluent hypergeometric function and $\psi_0(x)=\Gamma'(x)/\Gamma(x)$ is the digamma function. 
...
We have developed a non-perturbative approach to find the spectrum of two interacting particles in a harmonic trapping potential with spin-orbit coupling. This approach is shown to be effective except in the case where the two-body interaction generates a small, positive scattering length. It can easily be adapted to calculate energies for bosonic systems, or to new forms of SOC such as the recently proposed spin-orbital angular momentum coupling \cite{2014arXiv1411.1737S}.
...
\begin{equation}
\int_0^\infty \frac{1}{z}\left[W_{\kappa,\mu}(z) \right]^2  dz=\frac{\pi}{\sin (2\pi \mu)}\frac{\psi(\frac{1}{2}+\mu-\kappa)-\psi(\frac{1}{2}+\mu-\kappa)}{\Gamma(\frac{1}{2}+\mu-\kappa)\Gamma(\frac{1}{2}-\mu-\kappa)},\quad |\operatorname{Re} \mu\;|<1/2
\end{equation}
